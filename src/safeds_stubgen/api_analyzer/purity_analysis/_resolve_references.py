...
class ReferenceResolver:
    ...
    api_data :
        API Data generated by api_analyzer for improved purity analysis
    ...
    @staticmethod
    def compare_amount_of_parameters(function: FunctionScope, call: astroid.Call) -> bool:
    ...
    def _get_function_scopes_by_call_reference(
    ...
    ) -> tuple[list[FunctionScope], bool]:
        """ Get list of FunctionScopes by accessing the api which was generated by the api_analyzer

        The api of the api_analyzer stores all functions and each function contains info about their
        body. The body contains info about each call reference and for each call reference, each
        possibly referenced function is stored. This information is used to reduce the list of 
        FunctionScopes. 

        Parameters
        ---------
        func : FunctionScope
            Holds information about the Function in which the call reference is made
        call_reference : Reference
            a call reference of the function "func"
        ...
        if self.old_purity_analysis:
            ...
            if self.evaluation is not None:
                self.evaluation.evaluate_call_reference(node_id.module, call_reference.id.name, [], result, call_reference.id.line, call_reference.id.col, False, False, True, False, False, False, [], None)
            return (result, False)
        ...
        if function_api is None or self.api_data.functions[function_id].body is None:  
            result = self._reduce_function_defs_by_parameter_comparison(function_defs, call_reference)
            if self.evaluation is not None:
                self.evaluation.evaluate_call_reference(node_id.module, call_reference.id.name, [], result, call_reference.id.line, call_reference.id.col, False, False, False, True, False, False, [], None)
            return (result, False)
        ...
        if self.evaluation is not None:
            ...
            if len(reduced_function_defs) == 0:
                pass
            if len(reduced_function_defs) < len(function_defs_old):
                ...
                self.evaluation.evaluate_call_reference(node_id.module, call_reference.id.name, reduced_function_defs, function_defs_old, call_reference.id.line, call_reference.id.col, False, True, False, False, False, False, call_reference_api.receiver.path_to_call_reference, call_reference_api.receiver.type)
            else:
                # type aware purity analysis found same amount of functions
    ...
    def _reduce_function_defs_by_parameter_comparison(self, function_defs: list[FunctionScope], call_reference: Reference):
        ...
        Parameters
        ---------
        function_defs : list[FunctionScope]
        call_reference : Reference
        ...
        list[FunctionScope]
        """
        function_defs = [function_d for function_d in function_defs if self.compare_amount_of_parameters(function_d, call_reference.node)]  # type: ignore[union-attr]
        return function_defs

    def _get_id_from_nodeId(self, nodeId: NodeID) -> str:
        """ Helper function for _get_function_scopes_by_call_reference

        Mypy and Astorid generate different ids, so we need to convert them
    ...
    def _get_id_from_api_function(self, function: Function) -> str:
        """ Helper function for _get_function_scopes_by_call_reference
        ...
        str
        """
        return f"{function.module_id_which_contains_def}.{function.name}.{function.line}.{function.column}"
    ...
    def _find_referenced_functions(
        ...
        # if type-aware purity analysis finds referenced functions we dont have to look at the other if cases
        # this should improve time complexity and reduces wrongly passed impurity reasons
        found_functions_through_type = False
        ...
        if call_reference.name in self.functions:
            ...
            function_def, found_functions_through_type = self._get_function_scopes_by_call_reference(function, call_reference)
            function_symbols = [func.symbol for func in function_def if function_def]  # type: ignore[union-attr]

            if found_functions_through_type:
        ...
        elif call_reference.name in self.classes and not found_functions_through_type:
        ...
        if call_reference.name in function.parameters and not found_functions_through_type:  # callbacks
                            ...
                            if call_references_result.referenced_symbols:
                                ...
                                for referenced_symbol in call_references_result.referenced_symbols:
                                    ...
                                    if referenced_symbol not in raw_reasons[function.symbol.id].calls:
                                        raw_reasons[function.symbol.id].calls.add(referenced_symbol)  # pm: for each function inside of the functionScope we add the referenced call to reasons
