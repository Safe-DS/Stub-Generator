...
from ._types import BoundaryType
...
class BoundaryList:
    ...
    def add_boundary(self, match_label: str, type_: str, match_string: Span | None = None) -> None:
...
def _analyze_matches(matches: list[tuple[str, Span]], boundaries: BoundaryList) -> None:
    ...
    type_id = 0
    other_id = 0
    processed_matches: list[dict[str, (int | str | Span)]] = []
    found_type = False
...
def extract_boundary(description: str, type_string: str) -> set[BoundaryType]:
    ...
    desc_matches: list[tuple[str, Span]] = []
    ...
    if type_matches:
        type_list: list[str] = []  # Possible numeric data types that may be used with the parameter to be examined.
        restriction_list: list[tuple[str, Span]] = []  # Restrictions of the type such as non-negative
        match_label = ""
